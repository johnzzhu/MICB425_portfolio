...
community_data = data.frame(
  ...
  occurences = c(101,18,24,16,241,174,187,3,3,2,6,3,14,9)
)
```
...
``` {r}
simpson_D = 0
community_count = sum(community_data$occurences)
for (i in 1:length(community_data$number)) {
  simpson_D = simpson_D + (community_data$occurences[i] / community_count) ^ 2
}
1 / simpson_D
```
...
for (i in 1:length(community_data$number)) {
  if (community_data$occurences[i] == 1) {
